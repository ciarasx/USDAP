...
from model.SFUniDA_pls import SFUniDA_PLS
...
def data_load(args):
    ...
    dset_loaders["strong_aug"] = DataLoader(dsets["strong_aug"], batch_size=test_bs, shuffle=True, num_workers= args.num_workers, drop_last=True)
...
def obtain_global_pseudo_labels(args, model, dataloader, epoch_idx=0.0):
    ...
    for _, imgs_test, imgs_label, _ in tqdm(dataloader, ncols=60):
        ...
        embed_feat, pred_cls, _ = model.forward_pls(imgs_test, apply_softmax=True)
    ...
    pos_cls_prior = torch.mean(sorted_pred_cls[:(pos_topk_num), :], dim=0, keepdim=True).t() * (
                1.0 - args.rho) + args.rho

    # 每个类别的先验概率
    args.logger.info(
        "POS_CLS_PRIOR:\t" + "\t".join(["{:.3f}".format(item) for item in pos_cls_prior.cpu().squeeze().numpy()]))
    ...
    psd_label_prior = torch.zeros_like(psd_label_prior_simi).scatter(1, psd_label_prior_idxs,
                                                                     1.0)  # one_hot prior #[N, C]
...
def train(args, model, pretrained_params, train_dataloader, test_dataloader, optimizer, epoch_idx=0.0):
    ...
    # amp_grad_scaler = GradScaler()
    #
    # trs = strong_augment(resize_size=256, crop_size=224, alexnet=False)
    for imgs_train, _, _, imgs_idx in tqdm(train_dataloader, ncols=60):
        ...
        # imgs_train = imgs_train.strong_augment()
        # inputs_train_stg = inputs_train_stg.cuda()
        # print (inputs_test_wk.shape)
        # print (inputs_test_stg.shape)
        # inputs_test = inputs_test_stg

        hard_psd_label = hard_psd_label_bank[imgs_idx]  # [B, C]

        embed_feat, pred_cls, bn_f = model.forward_pls(imgs_train, apply_softmax=True)

        bn_pred_loss = bn_loss(model, pretrained_params, bn_f, alpha=0.01, i=3)
    #
    ...
    #
        loss = bn_pred_loss + psd_pred_loss
        # loss = bn_pred_loss
        #
...
def test(args, model, dataloader, src_flg=False):
    ...
    for _, imgs_test, imgs_label, _ in tqdm(dataloader, ncols=60):
        imgs_test = imgs_test.cuda()
        _, pred_cls, _ = model.forward_pls(imgs_test, apply_softmax=True)
...
def main(args):
    os.environ["CUDA_VISIBLE_DEVICES"] = args.gpu  #指定运行代码gpu设备
    this_dir = os.path.join(os.path.dirname(__file__), ".")  #获取当前脚本文件所在的目录路径，并将其赋值给变量 this_dir，以便后续使用

    model = SFUniDA_PLS(args)
    ...
    args.logger = set_logger(args, log_name="log_PLS_training.txt")

    # 网络参数，需要得到源模型BN的参数，用来计算和动态的BN参数差值
    param_group = []
    ...
    param_group += [{'params': model.data_prompt, 'lr': args.lr}]
    ...
    for epoch_idx in tqdm(range(args.epochs), ncols=60):
        # Train on target
        # model.load_state_dict(checkpoint["model_state_dict"], False)
        loss_dict = train(args, model, pretrained_params, target_train_dataloader, target_test_dataloader, optimizer, epoch_idx)
        ...
        prompt_file = "prompt_model.pth"
        torch.save({
            "epoch": epoch_idx,
            "model_state_dict": model.state_dict()}, os.path.join(save_dir, checkpoint_file))
        torch.save({
            "epoch": epoch_idx,
            "data_prompt": model.data_prompt.detach().cpu()
        }, os.path.join(save_dir, prompt_file))
