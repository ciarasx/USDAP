...
def test(args, model, dataloader, src_flg=True):
    ...
    np.save("embed_feat_officehome.npy", embed_feat_all)
    np.save("label_officehome.npy", label_all)

    h_score, known_acc, unknown_acc, per_cls_acc = compute_h_score(args, class_list, gt_label_all, pred_cls_all,
                                                                   open_flg, open_thresh=0.50)
...
def main(args):
    ...
    source_test = DataLoader(source_dataset, batch_size=args.batch_size, shuffle=False,
                                   num_workers=args.num_workers, drop_last=True)
    ...
    for epoch_idx in tqdm(range(args.epochs), ncols=60):
        ...
        if epoch_idx % 1 == 0:
            # EVALUATE ON SOURCE
            source_h_score, source_known_acc, source_unknown_acc, src_per_cls_acc = test(args, model, source_test, src_flg=True)
